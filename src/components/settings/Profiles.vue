        ...
        <table class="styled-table" v-if="!loading && profiles.length > 0">
            ...
            <tbody>
                <tr v-for="profile in profiles" :key="profile.id">
                    ...
                    <td>
                        <button class="edit-profile-button" @click="openEditModal(profile)">
...
interface EditableProfile {
    id: number;
    name: string;
    permissions: number[]; // Permissões como IDs para o modal
}
...
import ModalProfiles from '../modals/ModalProfiles.vue';
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            loading: true,
            showModal: false,
            selectedProfile: null as EditableProfile | null, // Perfil selecionado com permissões como IDs
        };
    },
    async mounted() {
        try {
    ...
    methods: {
        openAddModal() {
            this.selectedProfile = null; // Limpa o perfil selecionado
            this.showModal = true;
        },
        openEditModal(profile: Profile) {
            // Converte o perfil para o formato esperado pelo modal
            this.selectedProfile = {
                id: profile.id,
                name: profile.name,
                permissions: profile.permissions.map((permission) => permission.id), // Apenas IDs
            };
            this.showModal = true;
        },
        closeModal() {
            this.showModal = false;
            this.selectedProfile = null; // Reseta o perfil ao fechar o modal
        },
